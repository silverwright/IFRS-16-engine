...
export function ContractSelector({ onSelect }: ContractSelectorProps) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Select a Contract</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {contracts.map((contract) => (
          <button
            ...
            className="bg-slate-50 dark:bg-white/5 backdrop-blur-sm border border-slate-300 dark:border-white/10 rounded-lg p-4 hover:border-blue-500 dark:hover:border-blue-400/50 hover:bg-slate-100 dark:hover:bg-white/10 hover:shadow-xl transition-all text-left group"
          >
            <div className="flex items-start gap-3 mb-3">
              <div className="w-10 h-10 bg-blue-100 dark:bg-blue-500/20 rounded-lg flex items-center justify-center group-hover:bg-blue-200 dark:group-hover:bg-blue-500/30 transition-colors">
                <FileText className="w-5 h-5 text-blue-600 dark:text-blue-400" />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="font-semibold text-slate-900 dark:text-white truncate">{contract.contractId}</h4>
            ...
            <div className="space-y-2 text-sm text-slate-600 dark:text-white/80">
              ...
              <div className="flex items-center gap-2">
                <Package className="w-4 h-4 text-slate-500 dark:text-white/60" />
            ...
            <div className="mt-3 pt-3 border-t border-slate-300 dark:border-white/10 text-xs text-slate-600 dark:text-white/70">
              Mode: <span className="font-medium text-slate-900 dark:text-white/90">{contract.mode}</span>
            </div>
          </button>
        ))}
      </div>
