 ...
 *
 * This module generates HTML-formatted lease agreements based on lease data.
 * It supports two modes:
 *
 * 1. **MINIMAL Mode**: A simplified lease agreement suitable for straightforward leases
 *    - Contains essential commercial terms and standard clauses
 *    - Faster to review and execute
 *    - Suitable for standard equipment leases
 *
 * 2. **FULL Mode**: A comprehensive lease agreement with detailed schedules
 *    - Includes all party details, addresses, and registration numbers
 *    - Contains detailed insurance, compliance, and ESG requirements
 *    - Multiple schedules (Acceptance Certificate, ESG Undertakings, etc.)
 *    - Suitable for high-value or complex leases requiring extensive documentation
 *
 * The generated HTML is styled for printing and can be used as a preview or
 * exported to PDF for execution.
 *
 ...
 *
 * Uses international number formatting (comma thousands separator).
 *
 ...
 * @example
 * ```typescript
 * formatCurrency(10000)    // Returns: "10,000.00"
 ...
 *
 * Uses British (en-GB) locale for date formatting.
 *
...
/**
 * Generate HTML contract document from lease data
 *
 * This function generates a complete, styled HTML lease agreement ready for
 * preview, printing, or PDF export. The contract includes:
 * - Header with contract ID and date
 * - Schedule of key commercial terms
 * - Standard lease clauses (17 sections)
 * - Signature blocks for both parties
 * - System-generated disclaimer
 *
 * The FULL mode additionally includes:
 * - Party registration and address details
 * - Detailed insurance schedules
 * - ESG and compliance undertakings
 * - Acceptance certificate template
 * - Purchase option and RVG details
 *
 * @param leaseData - Partial lease data (only required fields need to be populated)
 * @param mode - Contract mode: 'MINIMAL' for basic contract or 'FULL' for comprehensive
 * @returns HTML string containing the complete formatted contract
 *
 * @example
 * ```typescript
 * const contractHtml = generateContractHTML(leaseData, 'MINIMAL');
 * // Use contractHtml in an iframe, export to PDF, or print
 * ```
 */
export function generateContractHTML(leaseData: Partial<LeaseData>, mode: 'MINIMAL' | 'FULL'): string {
   ...
   *
   * Simplified lease agreement with essential terms and standard clauses.
   * Suitable for straightforward equipment leases.
   * ============================================================================ */
  ...
  if (mode === 'MINIMAL') {
    return `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        ...
        <h2 style="color: #1e40af; margin-top: 1.5rem; font-size: 18px; font-weight: bold;">Schedule — Key Commercial Terms</h2>
   ...
   * FULL MODE CONTRACT
   *
   * Comprehensive lease agreement with detailed schedules and extensive clauses.
   * Includes party details, insurance schedules, ESG undertakings, acceptance
   * certificate, and purchase option/RVG details.
   * ============================================================================ */
  ...
  return `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      ...
      <h2 style="color: #1e40af; margin-top: 1.5rem; font-size: 18px; font-weight: bold;">Schedule 1 — Key Commercial Terms</h2>
      <table style="width: 100%; border-collapse: collapse; margin: 1rem 0;">
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #ddd;">
            Renewal: ${leaseData.RenewalOptionYears || 0}.0 years (likelihood for reporting: ${leaseData.RenewalOptionLikelihood || 0}%).
            Termination option point: ${leaseData.TerminationOptionPoint || 'N/A'} (penalty ${leaseData.Currency || 'NGN'} ${formatCurrency(leaseData.TerminationPenaltyExpected || 0)}).
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #ddd;">
            ${leaseData.EscalationType || 'CPI-linked'} (Base ${leaseData.BaseCPI || 0}); reset month ${leaseData.CPIResetMonth || 1}; first reset +${leaseData.FirstResetYearOffset || 1} year(s).
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #ddd;">
            ${formatCurrency(leaseData.PurchaseOptionPrice || 0)} / ${formatCurrency(leaseData.RVGExpected || 0)} (RVG reasonably certain? ${leaseData.RVGReasonablyCertain ? 'Yes' : 'No'})
          </td>
        </tr>
        <tr>
      ...
      <h2 style="color: #1e40af; margin-top: 1.5rem; font-size: 18px; font-weight: bold;">Schedule 2 — Lease Agreement</h2>

      <h2 style="color: #black; font-weight: bold;">1. Parties and Definitions</h2>
      <p style="margin-bottom: 1.5rem;">This agreement (the "Agreement") is between <strong>${leaseData.LessorName || 'Lessor'}</strong> (the "Lessor") and <strong>${leaseData.LesseeEntity || 'Lessee'}</strong> (the "Lessee").</p>

      <h2 style="color: #black; font-weight: bold;">2. Lease and Title</h2>
      ...
      <p style="margin-bottom: 1.5rem;">2.2 No right, title or interest passes to Lessee other than the leasehold interest. The Asset is a chattel separate from any site.</p>

      <h2 style="color: #black; font-weight: bold;">3. Delivery, Commissioning and Acceptance</h2>
      ...
      <p>3.2 Risk Transfer: Risk passes upon Signing of Acceptance Certificate; title remains with Lessor.</p>
      <p style="margin-bottom: 1.5rem;">3.3 Acceptance: Upon successful commissioning and Lessee's execution of the Acceptance Certificate (Schedule 4), the Asset is deemed accepted. If defects are identified, Lessor shall rectify within a reasonable cure period; acceptance follows re-test.</p>

      <h2 style="color: #black; font-weight: bold;">4. Rent; Payment Mechanics; Escalation</h2>
      ...
      <p>4.2 Late amounts accrue default interest at the maximum rate permitted by law.</p>
      <p style="margin-bottom: 1.5rem;">4.3 Escalation: Rent is adjusted by reference to ${leaseData.EscalationType || 'CPI'} (Base ${leaseData.BaseCPI || 0}), reset in month ${leaseData.CPIResetMonth || 1}, first reset ${leaseData.FirstResetYearOffset || 1} year(s) after commencement. If CPI is discontinued or materially changed, the Parties shall in good faith select a successor index that most closely reflects CPI movements.</p>

      <h2 style="color: #black; font-weight: bold;">5. Taxes; Withholding; Gross-Up</h2>
      <p style="margin-bottom: 1.5rem;">All amounts are exclusive of taxes. If Lessee is required by law to withhold or deduct taxes from a payment, Lessee shall gross-up so that Lessor receives the amount it would have received absent such withholding, except for taxes on Lessor's net income.</p>

      <h2 style="color: #black; font-weight: bold;">6. Use; Maintenance; Relocation; Software</h2>
      <p>6.1 Lessee shall use the Asset solely for lawful business purposes and keep it in good working order per manufacturer specs.</p>
      <p>6.2 No relocation without Lessor consent.</p>
      <p style="margin-bottom: 1.5rem;">6.3 Embedded software is licensed on a non-exclusive, non-transferable, term-limited basis. Lessee shall not tamper with firmware or controls.</p>

      <h2 style="color: #black; font-weight: bold;">7. Insurance</h2>
      <p style="margin-bottom: 1.5rem;">Lessee shall insure the Asset for not less than ${leaseData.Currency || 'NGN'} ${formatCurrency(leaseData.InsuranceSumInsured || 0)} and maintain third-party liability of at least ${leaseData.Currency || 'NGN'} ${formatCurrency(leaseData.InsuranceTPLimit || 0)} with an insurer rated at least "${leaseData.InsurerRatingMin || 'A'}".</p>

      <h2 style="color: #black; font-weight: bold;">8. Compliance Undertakings</h2>
      <p style="margin-bottom: 1.5rem;">Lessee shall comply with applicable law, maintain anti-bribery and sanctions controls, and operate the Asset in line with ESG requirements (Schedule 5).</p>

      <h2 style="color: #black; font-weight: bold;">9. Events of Default; Remedies</h2>
      <p style="margin-bottom: 1.5rem;">Events include non-payment, breach, insolvency, unlawful use, failure to insure, or unauthorised relocation. On default, Lessor may terminate, demand payment, repossess, and claim damages.</p>

      <h2 style="color: #black; font-weight: bold;">10. Risk; Loss and Damage</h2>
      <p style="margin-bottom: 1.5rem;">From risk transfer, Lessee bears risk of loss or damage (except Lessor's wilful misconduct). Insurance proceeds are applied to repair or settlement.</p>

      <h2 style="color: #black; font-weight: bold;">11. Assignment and Sub-leasing</h2>
      <p style="margin-bottom: 1.5rem;">Lessee may not assign or sub-lease without consent. Lessor may assign or finance rights with notice.</p>

      <h2 style="color: #black; font-weight: bold;">12. Data; Metering; Audit Rights</h2>
      <p style="margin-bottom: 1.5rem;">Where the Asset generates data, Lessee grants Lessor access for maintenance and billing. Lessee shall permit reasonable audits upon notice.</p>

      <h2 style="color: #black; font-weight: bold;">13. Force Majeure</h2>
      <p style="margin-bottom: 1.5rem;">Neither Party is liable for failure or delay caused by events beyond its control, provided mitigation efforts are made.</p>

      <h2 style="color: #black; font-weight: bold;">14. Dispute Resolution; Governing Law</h2>
      <p style="margin-bottom: 1.5rem;">Disputes are referred to arbitration under ${leaseData.ArbitrationRules || 'Lagos Court of Arbitration Rules'}, seat Lagos, Nigeria, language English. This Agreement is governed by ${leaseData.GoverningLaw || 'Laws of the Federal Republic of Nigeria'}.</p>

      <h2 style="color: #black; font-weight: bold;">15. Notices</h2>
      <p style="margin-bottom: 1.5rem;">Notices shall be delivered to addresses in Schedule 1. Email may be used for operational communications but not for service of proceedings unless agreed.</p>

      <h2 style="color: #black; font-weight: bold;">16. Entire Agreement; Variation; Severability</h2>
      <p style="margin-bottom: 1.5rem;">This Agreement (including Schedules) is the entire agreement. Amendments must be in writing signed by both Parties. Invalid provisions do not affect the remainder.</p>

      <h2 style="color: #black; font-weight: bold;">17. Signatures</h2>
      <table style="width: 100%; border: none; margin: 0.5rem 0;">
        <tr><td style="border: none; padding: 4px 0; font-weight: bold;">Lessor:</td><td style="border: none; padding: 4px 0;">${leaseData.LessorName || 'N/A'}</td></tr>
        <tr><td style="border: none; padding: 4px 0;"></td><td style="border: none; padding: 4px 0;">By: ____________________</td></tr>
        <tr><td style="border: none; padding: 4px 0;"></td><td style="border: none; padding: 4px 0;">Name: ____________________</td></tr>
      ...
      <h2 style="color: #1e40af; margin-top: 1.5rem; font-size: 18px; font-weight: bold;">Schedule 4 — Acceptance Certificate (Template)</h2>
      ...
      <p style="margin-bottom: 2rem;"></p>
      <h2 style="color: #1e40af; margin-top: 1.5rem; font-size: 18px; font-weight: bold;">Schedule 5 — ESG & Compliance Undertakings</h2>
      <ul>
      ...
      <p style="margin-bottom: 2rem;"></p>
      <h2 style="color: #1e40af; margin-top: 1.5rem; font-size: 18px; font-weight: bold;">Schedule 6 — Purchase Option / Residual Value Guarantee</h2>
      <p>Purchase Option Price: ${formatCurrency(leaseData.PurchaseOptionPrice || 0)}. Exercise mechanics, timing and conditions precedent to be set out in the implementing notice. RVG (if any) applies as per Schedule 1; calculation methodology to be appended if required.</p>

      <p style="margin-bottom: 2rem;"></p>
      <h2 style="color: #1e40af; margin-top: 1.5rem; font-size: 18px; font-weight: bold;">Schedule 7 — Insurance Details</h2>
