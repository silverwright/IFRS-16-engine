  ...
  return (
    <div className="w-full min-h-screen p-6 space-y-6 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Reports & Disclosures</h1>
          <p className="text-white/80">Generate IFRS 16 compliant reports and disclosures</p>
        </div>
      </div>
      ...
      {calculations && (
        <div className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-6 shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-white">{selectedReport} Preview</h3>
            <Button
              variant="outline"
          ...
          {selectedReport === 'Maturity Analysis' && (
            <div className="space-y-4">
              <h4 className="font-semibold text-white/90">Lease Liability Maturity Analysis</h4>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-white/10">
                  ...
                  <tbody className="bg-white/5 divide-y divide-white/10">
                    <tr>
                      <td className="px-4 py-3 text-sm text-white">Year 1</td>
                    ...
                    <tr className="bg-white/10">
                      <td className="px-4 py-3 text-sm text-white">Years 2-5</td>
                      <td className="px-4 py-3 text-sm text-white">
          ...
          {selectedReport === 'Journal Entries Summary' && (
            <div className="space-y-4">
              <h4 className="font-semibold text-white/90">Journal Entries Summary</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h5 className="font-medium text-white/90 mb-2">Initial Recognition</h5>
                  <div className="bg-white/5 p-4 rounded-lg space-y-2 text-sm">
                    <div className="text-white/90">Dr. Right-of-use Asset: {leaseData.Currency} {calculations.initialROU.toLocaleString()}</div>
                    <div className="text-white/90">Cr. Lease Liability: {leaseData.Currency} {calculations.initialLiability.toLocaleString()}</div>
                  </div>
                </div>
                <div>
                  <h5 className="font-medium text-white/90 mb-2">Annual Impact (Estimated)</h5>
                  <div className="bg-white/5 p-4 rounded-lg space-y-2 text-sm">
                    <div className="text-white/90">Interest Expense: {leaseData.Currency} {(calculations.totalInterest / (leaseData.NonCancellableYears || 1)).toLocaleString()}</div>
                    <div className="text-white/90">Depreciation Expense: {leaseData.Currency} {(calculations.totalDepreciation / (leaseData.NonCancellableYears || 1)).toLocaleString()}</div>
                  </div>
                </div>
              </div>
            </div>
